...
```{r }
mymat <- matrix(2:10, ncol = 3, nrow = 3)
mymat
```

```{r }
# variable concatinated from name variable
myname = name
myname
...
emp_status <- factor(c("Employed", "Unemployed","Employed",
                      "Self-Employed","Employed","Self-Employed","Unemployed"))

```


```{r }
# displaying levels through table
emp_status <- factor(c("Employed", "Unemployed","Employed",
                      "Self-Employed","Employed","Self-Employed","Unemployed"))
table(emp_status)
...
CustomerID <- c("Alicia", "Nancy", "Fernando","Loius","Diano")
...
number2 <- as.integer(readline(prompt = "Enter Second numberr"))
...
allConditionsMeet <- funtion(number1,number2){
...
# element wise multiplication is not compatible 
...
curr_date <- as.POSIXlt(Sys.time())
namess <- unclass(curr_date)
...
```{r }
curr_time <- Sys.time()
mytime <- as.POSIXlt(curr_time,tz = "GMT")
diff = curr_time - mytime
```
...
lapply(numbers, mymean)
```


```{r }
sapply(numbers, mean)
sapply(numbers, mystd)
```
...
```{r }
head(Orange)
split_dataaa <- split(Orange, Orange$circumference)
```


```{r }
split_dataaa <- split(Orange, Orange$circumference)
print(split_dataaa$`30`)
print(split_dataaa$`75`)
```
